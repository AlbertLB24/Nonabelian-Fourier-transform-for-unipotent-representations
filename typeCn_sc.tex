...
\end{lemma}
\begin{proof}
    The first part can be found in \textcolor{red}{Collingwood}, while the second one can be easily checked with a computer algebra program (such as GAP).
\end{proof}
...
\end{equation}
To calculate the $W_{J_r}$-modules above, we first calculate the characters $\chi_{t_1}^{J_r}$ and $\chi_{t_1^\eta}^{J_r}$. Using \textcolor{red}{Reeder}, we can show that $\chi_{t_1^\eta}^{J_r}$ is the trivial character. On the other hand, $W_{J_r,t_1}\cong\langle s_0\rangle\times W(B_{r-2})\times W(B_{n-r})$ as a subgroup of $W_{J_r}\cong W(B_{r})\times W(B_{n-r})$, and $\chi_{t_1}^{J_r}$ is trivial on $W(B_{r-2})\times W(B_{n-r})$ while $\chi_{t_1}^{J_r}(s_0)=-1$.
...
\begin{proposition}[Proof in progress. It relies on the generalized Springer correspondence]
    Let $\theta_r'$ be the irreducible representation of $\Sp_{2r}(\FF_q)$ labelled by the symbol
    $$\begin{pmatrix}
        0 & 1 & 2 & \ldots & r-2 & r-1 & r\\
        && 1 & \ldots & r-2 &&
    \end{pmatrix}.$$
    Then the parahoric restrictions of $\pi(ut_2,\varepsilon\otimes\mathbf{1})$ and $\pi(ut_2,\varepsilon\otimes\varepsilon)$ with respect to $K_{J_r}$ are irreducible representations labelled by $\theta_r'\boxtimes(\emptyset,1^{n-r})$ and $(\emptyset,1^r)\boxtimes\theta_{n-r}'$, respectively.
\end{proposition}
