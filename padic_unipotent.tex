\section{Unipotent representations of \texorpdfstring{$p$}{PDFstring}-adic groups}\label{sec:padic_unipotent}
...
Throughout, we let $F$ be a nonarchimedean local field with ring of integers $\cO$, uniformizer $\varpi$ and residue field $k$ of cardinality $q$, a power of a prime $p$. Let $\mathbf{G}$ be a connected, almost simple, split algebraic group over $F$ and let $G=\mathbf{G}(F)$ be the group of $F$-rational points. In this section, we will first discuss some results about the structure theory of the group $G$, including the Bruhat--Tits building and the classification of parahoric (or open compact) subgroups of $G$. There are many texts explaining these topics, from the foundational papers of Bruhat and Tits (\cite{BruhatTits1972} and \cite{BruhatTits1984}) to more modern expositions (\cite{RabinoffBruhatTits} is a brilliant exposition for this theory applied to Chevalley groups).

These results are instrumental to study and classify smooth admissible complex unipotent representations of $G$, the main object of study in the local Langlands correspondence. We recall that a representation $(\pi,V)$ is smooth if the stabilizer of each $v\in V$ in $G$ is open and is admissible if the fixed points $V^K$ for any open compact subgroup of $G$ is finite dimensional. In sections \ref{subsec:unipotent_reps} - \ref{subsec:dualFourier} we will use these results to study, in particular, unipotent representations of $G$, defined in \ref{subsec:unipotent_reps}.
...
\begin{example}
    \begin{enumerate}
        \item Let $G=\SL_2(F)$ and $T$ be the set of diagonal matrices. Then $\Phi(G,T)=\{\pm\alpha\}$ where 
        \[
        ...
        \item Let $G=\PGL_2(F)$ and $T$ be the set of diagonal matrices. Then $\Phi(G,T)=\{\pm\alpha\}$ where 
        \[
            \alpha\begin{psmallmatrix}
                t & 0\\
                0 & 1
        ...
        \item Let $G=\GL_2(F)$ and $T$ be the set of diagonal matrices. Then $\Phi(G,T)=\{\pm\alpha\}$ where 
        \[
            \alpha\begin{psmallmatrix}
                t & 0\\
                0 & s
...
Some of the behaviour observed in the previous examples holds in much greater generality. For example, $\Omega$ is an abelian group, and it has finite order if and only if $G$ is a simple group. In that case, $\Omega$ is in bijection with the centre of complex dual group $G^\vee(\CC)$ of $G$ and with the length preserving automorphisms of the affine Dynkin diagram (see \cite[\S1.8]{IwahoriMatsumoto1965}). In particular, if $G$ is simply connected, then $\Omega$ is trivial, while $\Omega$ has the largest size within the isogeny class when $G$ is adjoint. On the other hand, $W_{\aff}$ only depends on the isogeny class, and therefore only on the root system of $G$.

\subsection{The Bruhat--Tits building and parahoric subgroups}

When we work with the apartment $\cA(G,T)$, we are making an arbitrary choice for the torus. It is possible, however, to push this idea further and construct the \textit{Bruhat--Tits} building $\cB(G)$, a canonical polysimplial space associated to $G$ that contains $\cA(G,T)$ for any $F$-split maximal torus. This is achieved by gluing together the apartments of all $F$-split maximal tori of $G$ according to some natural equivalence relation. An important property of the building $\cB(G)$ is that it carries a $G$-action satisfying the following:
\begin{enumerate}
...
\begin{example}
    ...
    \end{psmallmatrix}\begin{psmallmatrix}
        ...
        p\ZZ_p & \ZZ_p^\times
    \end{psmallmatrix}_{\det\in\ZZ_p^\times},$$
    a disconnected open compact subgroup. We define the Iwahori subgroup $\cI$ to be the identity connected component of $K_{\mathcal{C}_0}^+$.
\end{example}
...
The groups $K_c^+$ will be of great importance since they bridge the representation theory of $p$-adic groups with that of finite groups of Lie type. To explain this connection, we note that there is a short exact sequence
...
In particular, if $c$ is open in the building, then $(K_c,U_c,\overline{K}_c)$ is a minimal parahoric subgroup. These are called an \textbf{Iwahori subgroups}, and they are all conjugate in $G$. The standard Iwahori subgroup $\cI$ corresponds to $J=\emptyset\subsetneq S_{\aff}$.
...
\subsection{Parahoric restriction and unipotent representations}\label{subsec:unipotent_reps}
In this section, we finally focus our attention in representation theoretic aspects of the $p$-adic group $G$, using the techniques developed in previous ones. The main object of study in the local Langlands correspondence is the category of smooth admissible complex representations of $G$, denoted by $\Rep(G)$. Let $\Irr(G)$ be the set of irreducible representations in $\Rep(G)$. This is a large set, and we are interested in a particular subset known as \textit{unipotent} irreducible representations of $G$ and denoted by $\Irr_{\un}(G)$, which we now define using parahoric subgroups.
...
\end{equation}
where $R(G)$ is the $\CC$-span of $\Irr(G)$ and $R(\overline{K})$ is the $\CC$-span of the irreducible representations of $\overline{K}$. This is well-defined since the representations are assumed to be admissible. The existence of such a functor is very powerful -- we can then apply the techniques of representation theory of finite groups of Lie type such as Deligne--Lusztig induction in the setting of $p$-adic groups. Let us begin with a natural definition.
...
\begin{example}
    ...
    $$\Irr_{\un}(G)=\Irr(G,\cI,\mathbf{1})\sqcup\bigsqcup_{\rho\in\{1,-1,\theta,\theta^2\}}\Irr(G,K_{J_0},G_2[\rho]).$$
...
Conversely, we would like to show that for any irreducible unipotent representation $(\pi,V)$ of $G$, the irreducible $\overline{K}$-submodules of $V^{U_K}$ are all unipotent, for any parahoric subgroup $(K,U_K,\overline{K}).$ This is a direct corollary of the following result.

\begin{proposition}\label{thm:unip_restriction}\cite[Lemma 5.1]{Reeder2000}
...
In this section, we give an overview of the Langlands parametrization of unipotent representations achieved by Lusztig in his celebrated paper \cite{Lusztig1995}. It relates the unipotent representations of $G$ with certain geometric structures of its complex dual group $G^\vee$. This is a complex reductive group, whose root datum is dual to that of $G$. Here are some examples for classical groups.
\begin{center}
...
This remarkable result, which we now study in some detail, fits naturally within the local Langlands correspondence and the theory of Langlands parameters (for a nice overview on the subject, we refer to \cite[\S 4]{AubertCiubotaruRomano2024} for a detailed explanation). Firstly, given a pair $(x,\phi)$, where $x\in G^\vee$ and $\phi\in\widehat{A_x}$, one can easily determine the inner twist $G'$ acting on the representation $(\pi_{(x,\phi)},V_{(x,\phi)})$. We consider $\phi$ as a character on $Z_{G^\vee}(x)$ and we restrict it to get a character of $Z(G^\vee)$. Tracing back through the bijection in \eqref{eqn:pureinnertwists} we obtain the corresponding pure inner twist $G'$.

Secondly, given a pair $(x,\phi)$ parametrizing a representation of $G$, we would like to know inside which unipotent block does $(\pi_{(x,\phi)},V_{(x,\phi)})$ lie. This is a much harder question to answer than the previous one, and its solution delves deep into the proof of the arithmetic-geometric correspondence. For the remainder of this section, we give an overview of some of the ideas involved which will be of use for us later. The main one is the use of \textit{Hecke algebras} to classify irreducible representation of an individual block. This approach has the added advantage that it reduces the problem of classifying infinite dimensional irreducible representation of $G$ of some block to classifying simple finite dimensional modules over the Hecke algebra. 
...
is a non-zero finite-dimensional module over the algebra $\cH(G,K,\sigma):=\End_G(\cInd_{K}^G \sigma)$ acting by precomposition, denoted the Hecke algebra associated to the pair $(K,\sigma)$.
\begin{theorem}
...
In the previous subsection we stated the arithmetic-geometric correspondence (also known as the unramified local Langlands correspondence), which reduces the classification of unipotent representations of $G$ to the classification of conjugacy classes of $G^\vee$ and the structure of the component group of their centralizers. To understand these, we first need to study the classification of unipotent conjugacy classes of $G^\vee$, an interesting problem in its own right that uncovers rich geometric structure inside $G^\vee$. For a full discussion on this topic, we refer to the book \cite{CollingwoodMcGovern1993}. We discuss, without proofs, some important facts that we will require later.
...
One can then picture this partial order in a diagram, called a \textit{Hasse diagram}, and one has the following generic picture. We also include some explicit examples for groups of types $G_2$, $C_3$ and $F_4$. These, and many others, can be found in \cite[\S 13.4]{Carter1993}
\begin{figure}[!ht]
    \centering
    \includegraphics{Unipotent structure.png}
    \caption{General structure of the ordering of unipotent classes (\cite[pg 55]{CollingwoodMcGovern1993}).}
\end{figure}
...
\begin{figure}[ht]
    ...
    \begin{minipage}{0.3\textwidth}
        \centering
        \includegraphics[width=0.5\textwidth]{G2_unipotentclasses.png}
        \caption{$G_2$ unipotent classes}
    \end{minipage}
    \hfill
    \begin{minipage}{0.3\textwidth}
        \centering
        \includegraphics[width=0.7\textwidth]{C3_unipotentclasses.png}
        \caption{$C_3$ unipotent classes}
    \end{minipage}
    \hfill
    \begin{minipage}{0.3\textwidth}
        \centering
        \includegraphics[width=\textwidth]{F4_unipotentclasses.png}
        \caption{$F_4$ unipotent classes}
    \end{minipage}
...
\newpage
\subsection{The dual nonabelian Fourier transform}\label{subsec:dualFourier}
At this stage, it is hopefully a natural question to ask whether there exists a linear involution $\FT^\vee:R_{\un}(G)\rightarrow R_{\un}(G)$ such that the square
\[
\begin{tikzcd}
...
\end{tikzcd}
\]
commutes, where the sum is taken over all maximal parahoric subgroups $K$. This is an open question in general, and a very difficult one. A significant amount of work has been done trying to define such a map with nice properties, most notably by Ciubotaru--Opdam \cite{CiubotaruOpdam2010}, Ciubotaru \cite{Ciubotaru2020} and Aubert--Ciubotaru--Romano \cite{AubertCiubotaruRomano2024}. The first important observation is that while the above map is hoped to exist for simply connected groups $G$, representations of pure inner twists of $G$ should interact with those of $G$, so a modification is required. More concretely, we consider instead the commutative square
\[
\begin{tikzcd}
\bigoplus_{G'}R_{\un}(G') \arrow[r,dashed, "\FT^\vee"] \arrow[d, "\res_{\un}^{\cpt}"] & \bigoplus_{G'}R_{\un}(G') \arrow[d, "\res_{\un}^{\cpt}"] \\
\bigoplus_{G'}\bigoplus_{K'} R_{\un}(\overline{K'}) \arrow[r, "\FT^{\cpt}_{\un}"] & \bigoplus_{G'}\bigoplus_{K'} R_{\un}(\overline{K'}),
\end{tikzcd}
\]
where the outer sum is taken over all pure inner twists of $G$ and the inner sum is taken over all \textbf{maximal open compact} subgroups of $G$, not necessarily connected. Their connected components at the identity are parahoric subgroups and they coincide with them when $G$ is simply connected. To simplify notation below, we let 
\begin{equation*}
    \mathcal{C}(G)_{\cpt,\un}:=\bigoplus_{G'\in\InnT^p(G)}\bigoplus_{K'\in\max(G')}R_{\un}(\overline{K}').
\end{equation*}

It is also important to highlight the fact that $\FT^{\cpt}_{\un}$ is generalization of Lusztig's nonabelian transform for finite groups of Lie type. For example, if a maximal open compact subgroup $K$ of $G$ has disconnected quotient $\overline{K}$, there is some other maximal open compact subgroup $K'$ of a pure inner twist $G'$ with disconnected quotient $\overline{K}'$ such that $\FT^{\mathrm{par}}$ acts on $R_{\un}(K)\oplus R_{\un}(K')$ but not on each individual term. We will not pursue this further since $G$ will always be simply connected in our examples, but a precise description can be found in \cite[\S 6]{AubertCiubotaruRomano2024}.
...
Definition \ref{def:ellFourier} allows us to state the main conjecture of this document (Conjecture \ref{conj:main} in the introduction), but we restate here it for completeness.

\begin{conjecture}(\cite[Conj. 9.7]{AubertCiubotaruRomano2024})
